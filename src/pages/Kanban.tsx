...
import { Calendar, MoreHorizontal, Plus, Instagram, Youtube, Linkedin, Mail, FileText, Music, MessageSquare, Clock, CheckCircle2, AlertCircle, Send, Eye, Zap, Trash2 } from 'lucide-react';
...
const columns: { id: Content['status']; title: string; icon: React.ReactNode; color: string }[] = [
  ...
  { id: 'Publicado', title: 'Publicados', icon: <Send size={18} />, color: 'text-sky-500 bg-sky-50 dark:bg-sky-900/20' },
];
      ...
      <div className="flex-1 overflow-x-auto pb-4">
        <DragDropContext onDragEnd={onDragEnd}>
          <div className="flex gap-6 h-full items-start">
            {columns.map(column => (
              <div key={column.id} className="w-80 flex-shrink-0 flex flex-col bg-gray-100/50 dark:bg-gray-800/30 rounded-xl h-full max-h-full overflow-hidden border border-gray-200 dark:border-gray-800">
                <div className={`p-4 border-b border-gray-200 dark:border-gray-800 flex justify-between items-center bg-white dark:bg-gray-900 rounded-t-xl`}>
                  <div className="flex items-center gap-2">
                    <span className={`p-1.5 rounded-lg ${column.color}`}>
                      {column.icon}
                    </span>
                ...
                <Droppable droppableId={column.id}>
                  {(provided, snapshot) => (
                    <div
                      ...
                      className={`flex-1 overflow-y-auto p-3 space-y-4 transition-colors ${snapshot.isDraggingOver ? 'bg-brand-primary/5' : ''} custom-scrollbar`}
                    >
                      ...
                      {boardData[column.id].map((content, index) => {
                        return (
                          // @ts-ignore - Library type mismatch with React 19 key prop
                          <Draggable key={content.id} draggableId={content.id} index={index}>
                            {(provided, snapshot) => (
                              <div
                                ...
                                className={`bg-white dark:bg-gray-900 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 group hover:border-brand-primary transition-all cursor-pointer flex flex-col ${snapshot.isDragging ? 'shadow-2xl ring-2 ring-brand-primary ring-opacity-50 rotate-2' : 'hover:shadow-md'}`}
                              >
                                <div className="flex justify-between items-start mb-3">
                                  <div className="flex flex-wrap gap-2">
                                    <span className={`flex items-center gap-1.5 text-[10px] font-bold px-2 py-1 rounded-lg uppercase tracking-wider ${
                                      content.channel === 'Instagram' ? 'bg-pink-50 dark:bg-pink-900/20 text-pink-600 dark:text-pink-400' :
                                      content.channel === 'TikTok' ? 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300' :
                                      content.channel === 'Blog' ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400' :
                                ...
                                <h4 className="text-base font-bold text-gray-900 dark:text-gray-100 mb-3 line-clamp-2 leading-snug group-hover:text-brand-primary transition-colors">{content.title}</h4>
                                ...
                                <div className="flex items-center justify-between mt-auto pt-3 border-t border-gray-100 dark:border-gray-800">
                                  ...
                                  <div className="flex items-center gap-1.5 text-gray-400 dark:text-gray-500 text-[10px] font-medium" title="Data de Publicação">
                                    <Calendar size={12} />
